...
const SectionBanner = ({ icon: Icon, number, title }) => (
  <div className="text-white px-4 py-2.5 flex items-center gap-2.5 mb-4" style={{ backgroundColor: "#153d63" }}>
    ...
    <div className="border border-slate-300 overflow-hidden">
      <div className="overflow-x-auto">
        <table className="w-full text-sm" style={{ tableLayout: "fixed" }}>
          <colgroup>
            {showResultadoColumn && <col style={{ width: "26%" }} />}
            <col style={{ width: showResultadoColumn ? "30%" : "56%" }} />
            <col style={{ width: "9%" }} />
            <col style={{ width: "9%" }} />
            <col style={{ width: "9%" }} />
            <col style={{ width: "9%" }} />
          ...
          <thead>
            <tr>
              {showResultadoColumn && (
                <th className="bg-slate-50 px-4 py-3 text-left align-bottom border-b border-r border-slate-300">
              ...
              <th colSpan={4} className="bg-slate-50 px-2 py-2 border-b border-slate-300 text-center">
                ...
                <div className="grid grid-cols-4 gap-0.5 mt-1.5">
                  {Object.values(CRITERIA).map((c) => (
                    <div key={c.label} className={`${c.bg} ${c.text} text-[10px] font-bold py-1`}>
                      {c.label}
                    </div>
                  ))}
                </div>
              </th>
              <th className="bg-slate-50 px-1 py-3 border-b border-slate-300 w-10">
            ...
            {rows.map((row) => (
              <tr key={row.id} className="hover:bg-slate-50/50">
                ...
                <td className="px-4 py-3 align-middle border-r border-slate-200">
                  ...
                  {row.temporalidad && (
                    <span className="inline-block mt-1 text-[10px] font-bold text-slate-500 bg-slate-100 border border-slate-200 px-2 py-0.5 uppercase tracking-wider">
                      {row.temporalidad}
                    </span>
                  )}
                </td>
                <td className={`px-2 py-3 text-center align-middle ${CRITERIA.ideal.soft} ${CRITERIA.ideal.chip} font-bold`}>
                  {row.ideal || "—"}
                </td>
                <td className={`px-2 py-3 text-center align-middle ${CRITERIA.esperado.soft} ${CRITERIA.esperado.chip} font-bold`}>
                  {row.esperado || "—"}
                </td>
                <td className={`px-2 py-3 text-center align-middle ${CRITERIA.intermedio.soft} ${CRITERIA.intermedio.chip} font-bold`}>
                  {row.intermedio || "—"}
                </td>
                <td className={`px-2 py-3 text-center align-middle ${CRITERIA.insuficiente.soft} ${CRITERIA.insuficiente.chip} font-bold`}>
                  {row.insuficiente || "—"}
                </td>
                <td className="px-1 py-3 text-center align-middle">
                  <button
                    ...
                    className="p-1.5 text-rose-500 hover:text-white hover:bg-rose-600 transition-colors"
                    title="Eliminar"
                  >
    ...
    <div className="bg-slate-50/70 border border-slate-200 rounded-xl p-4 space-y-3">
      ...
      <div className="flex justify-end">
        <button
          ...
          className="text-white text-xs font-semibold px-3 py-1.5 inline-flex items-center gap-1.5 hover:opacity-90 disabled:bg-slate-300 disabled:cursor-not-allowed transition-opacity"
          style={{ backgroundColor: !draft.resultado.trim() || !draft.kpi.trim() ? undefined : "#153d63" }}
        >
          <Plus className="w-3.5 h-3.5" strokeWidth={2.5} /> Añadir KPI
        </button>
      </div>
    </div>
  );
    ...
    <div className="bg-slate-50/70 border border-slate-200 rounded-xl p-4 space-y-3">
      ...
      <div className="flex justify-end">
        <button
          ...
          className="text-white text-xs font-semibold px-3 py-1.5 inline-flex items-center gap-1.5 hover:opacity-90 disabled:bg-slate-300 disabled:cursor-not-allowed transition-opacity"
          style={{ backgroundColor: !draft.descripcion.trim() ? undefined : "#153d63" }}
        >
          <Plus className="w-3.5 h-3.5" strokeWidth={2.5} /> Añadir OKR
        </button>
      </div>
    </div>
  );
