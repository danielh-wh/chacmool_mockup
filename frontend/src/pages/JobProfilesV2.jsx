      ...
      <div className="border border-slate-300 overflow-hidden bg-white">
        <div className="overflow-x-auto">
          <table className="w-full text-sm" style={{ tableLayout: "fixed" }}>
            <colgroup>
              {showResultadoColumn && <col style={{ width: "24%" }} />}
              <col style={{ width: showResultadoColumn ? "22%" : "36%" }} />
              <col style={{ width: "12%" }} />
              <col style={{ width: "9%" }} />
              <col style={{ width: "9%" }} />
              <col style={{ width: "9%" }} />
            ...
            <thead>
              <tr>
                {showResultadoColumn && (
                  <th className="bg-slate-50 px-3 py-3 text-left align-bottom border-b border-r border-slate-300">
                ...
                <th colSpan={4} className="bg-slate-50 px-2 py-2 border-b border-slate-300">
                  ...
                  <div className="grid grid-cols-4 gap-0.5">
                    {Object.values(CRITERIA).map((c) => (
                      <div key={c.label} className={`${c.bg} ${c.text} text-[10px] font-bold py-1 text-center`}>
                        {c.label}
                      </div>
                    ))}
                  </div>
                </th>
                <th className="bg-slate-50 px-1 py-3 border-b border-slate-300 w-10"></th>
              </tr>
            </thead>
            <tbody>
              {rows.length === 0 ? (
                ...
                rows.map((row) => (
                  <tr
                    key={row.id}
                    className="group border-t border-slate-200 hover:bg-slate-50/40 transition-colors"
                  >
                    {showResultadoColumn && (
                      <td className="border-r border-slate-200 p-0 align-middle">
                        <input
                    ...
                    {["ideal", "esperado", "intermedio", "insuficiente"].map((key) => {
                      ...
                      return (
                        <td key={key} className={`${c.soft} p-0 align-middle border-r border-white`}>
                          <input
                            value={row[key] || ""}
                            onChange={(e) => onUpdateRow(row.id, { [key]: e.target.value })}
                            placeholder="—"
                            className={`w-full bg-transparent px-1 py-3 text-center font-bold text-sm ${c.chip} placeholder:text-slate-400 placeholder:font-normal focus:outline-none focus:ring-2 focus:ring-inset ${c.ring}`}
                          />
                        </td>
                      );
                    })}
...
const JobProfileFormV2 = ({ initial, onCancel, onSave, companyValues }) => {
  // Para nuevos perfiles: pre-seleccionar TODOS los valores de empresa.
